    ...
    for operation in optimizedOperations(finalState.state.operations) {
        switch operation {
            ...
            case let .UpdateState(innerState):
                guard let currentState = transaction.getState() as? AuthorizedAccountState else {
                    break
                }
